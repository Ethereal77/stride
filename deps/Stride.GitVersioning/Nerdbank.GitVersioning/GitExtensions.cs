...
// Copyright (c) 2020 Andrew Arnott
...
namespace Nerdbank.GitVersioning
{
    ...
    public static class GitExtensions
    {
        ...
        /// <summary>
        ///   Maximum allowable value for the <see cref="Version.Build"/> and <see cref="Version.Revision"/> components.
        /// </summary>
        ...
        ///   ancestor (inclusive) that set the version to the value at <paramref name="commit"/>.
        /// </summary>
        ...
        public static int GetTruncatedCommitIdAsInt32(this Commit commit) => BitConverter.ToInt32(commit.Id.RawId, 0);
        ...
        public static ushort GetTruncatedCommitIdAsUInt16(this Commit commit) => BitConverter.ToUInt16(commit.Id.RawId, 0);
        ...
        /// <summary>
        ///   Encodes a commit from history in a <see cref="Version"/> so that the original commit can be found later.
        /// </summary>
        /// <param name="commit">The commit whose ID and position in history is to be encoded.</param>
        /// <param name="repoRelativeProjectDirectory">The repo-relative project directory for which to calculate the version.</param>
        /// <param name="versionHeight">
        ///   The version height, previously calculated by a call to <see cref="GetVersionHeight(Commit, string)"/> with the same
        ///   value for <paramref name="repoRelativeProjectDirectory"/>.
        /// </param>
        /// <returns>
        ///   A version whose <see cref="Version.Build"/> and <see cref="Version.Revision"/> components are calculated based on the commit.
        /// </returns>
        /// <remarks>
        ///   In the returned version, the <see cref="Version.Build"/> component is the height of the Git commit while the
        ///   <see cref="Version.Revision"/> component is the first four bytes of the Git commit id (forced to be a positive integer).
        /// </remarks>
        ...
        /// <summary>
        ///   Encodes HEAD (or a modified working copy) from history in a <see cref="Version"/> so that the original commit can be found later.
        /// </summary>
        ...
        public static Version GetIdAsVersion(this Repository repo, string repoRelativeProjectDirectory = null, int? versionHeight = null)
        {
            ...
            if (IsVersionFileChangedInWorkingCopy(repo, repoRelativeProjectDirectory, out _,  out VersionOptions workingCopyVersionOptions))
            {
                // Apply ordinary logic, but to the working copy version info
                return GetIdAsVersionHelper(headCommit, workingCopyVersionOptions, repoRelativeProjectDirectory, versionHeight);
            }
        ...
        /// <exception cref="ArgumentException">The provided path does not lead to an existing directory.</exception>
        ...
        {
            return TryHelpFindLibGit2NativeBinaries(basePath, out _);
        }
        ...
        /// <summary>
        ///   Tests whether a commit is of a specified version, comparing major and minor components with the version.txt file defined by that commit.
        /// </summary>
        ...
        private static bool CommitMatchesVersion(Commit commit, Version expectedVersion, string repoRelativeProjectDirectory)
        {
            ...
            return majorMinorFromFile?.Major == expectedVersion.Major &&
                   majorMinorFromFile?.Minor == expectedVersion.Minor &&
                   majorMinorFromFile?.Build == expectedVersion.Build &&
                   majorMinorFromFile?.Revision == expectedVersion.Revision;
        }
        ...
        /// <summary>
        ///   Gets the number of commits in the longest single path between the specified branch's head and the most
        ///   distant ancestor (inclusive).
        /// </summary>
        /// <param name="commit">The commit to measure the height of.</param>
        /// <param name="heights">A cache of commits and their heights.</param>
        /// <param name="continueStepping">
        ///   A function that returns <c>false</c> when we reach a commit that should not be included in the height calculation.
        ///   May be <c>null</c> to count the height to the original commit.
        /// </param>
        /// <returns>The height of the branch.</returns>
        private static int GetCommitHeight(Commit commit, Dictionary<ObjectId, int> heights, Func<Commit, bool> continueStepping)
        {
            ...
            {
                height = 0;
                if (continueStepping is null || continueStepping(commit))
                {
                    height = 1;
        ...
        private static IEnumerable<Commit> GetCommitsReachableFromRefs(Repository repo)
        {
            ...
            {
                if(reference.ResolveToDirectReference()?.Target is Commit commit)
                {
        ...
        /// <param name="set">The set into which the <paramref name="startingCommit"/> and all its ancestors are to be added.</param>
        ...
        private static Version GetIdAsVersionHelper(Commit commit, VersionOptions versionOptions, string repoRelativeProjectDirectory, int? versionHeight)
        {
            ...
            // The compiler (due to WinPE header requirements) only allows 16-bit version components, and forbids 0xFFFF as a value.
            //   The build number is set to the Git height. This helps ensure that within a major.minor release, each patch has an
        ...
        /// <summary>
        ///   Gets the version options from HEAD and the working copy (if applicable), and tests their equality.
        /// </summary>
        /// <param name="repo">The repo to scan for version info.</param>
        /// <param name="repoRelativeProjectDirectory">
        ///   The path to the directory of the project whose version is being queried, relative to the repo root.
        /// </param>
