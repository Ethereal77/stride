...
namespace Stride.Core.Diagnostics
{
    ...
    public abstract class LogListener : IDisposable
    {
        ...
        /// <summary>
        ///   Gets or sets a value indicating whether to flush the contents of the log asynchronously.
        /// </summary>
        /// <value><c>true</c> if [use flush async]; otherwise, <c>false</c>.</value>
        public bool UseFlushAsync { get; set; }

        /// <summary>
        ///   Gets or sets a function used to converts a <see cref="ILogMessage" /> instance into a text string.
        /// </summary>
        public Func<ILogMessage, string> TextFormatter { get; set; }

        /// <summary>
        ///   Gets or sets the max number of log messages that we should accumulate before a flush. Default is on every message.
        /// </summary>
        /// <value>Max number of log messages that we should accumulate before a flush.</value>
        /// <exception cref="ArgumentOutOfRangeException">The value must be greater than 0.</exception>
        public int LogCountFlushLimit
        {
            get => logCountFlushLimit;
            set
            {
                if (value <= 0)
                    throw new ArgumentOutOfRangeException(nameof(LogCountFlushLimit), "LogCountFlushLimit must be greater than 0.");
        ...
        /// <summary>
        ///   Called when a log message has been created.
        /// </summary>
        /// <param name="logMessage">The log message.</param>
        protected abstract void OnLog(ILogMessage logMessage);

        /// <summary>
        ///   Determines whether the log should be flushed after a log message. By default, flushing is occurring if the message has
        ///   a higher level than <see cref="LogMessageType.Info"/> or if we have accumulated more messages than the count specified
        ///   by <see cref="LogCountFlushLimit"/>.
        /// </summary>
        /// <param name="logMessage">The log message.</param>
        ...
        {
            // By default flush if we have more than the level info (Warning, Error, Fatal) or we have many accumulated messages
            return (logMessage.Type > LogMessageType.Info) || (LogMessageCount % LogCountFlushLimit) == 0;
        }

        /// <summary>
        ///   Flush the contents of the log. By default, this method does nothing. This method must be overriden in a subclass to
        ///   provide the flushing logic.
        /// </summary>
        ...
        /// </summary>
        /// <param name="logMessage">The log message.</param>
        /// <returns>A textual representation of the message.</returns>
        ...
        /// <returns>An <see cref="Action{ILogMessage}"/> result of the conversion.</returns>
        [NotNull]
