...
namespace Xenko.Assets.Presentation.AssetEditors.EntityHierarchyEditor.ViewModels
{
    public class EntityGizmosViewModel : DispatcherViewModel
    {
            ...
            : base(serviceProvider)
        {
            ...
            activeGizmos.AddRange(typeof(EntityComponent).GetInheritedTypes().Where(x => XenkoDefaultAssetsPlugin.GizmoTypeDictionary.ContainsKey(x)).OrderBy(DisplayAttribute.GetDisplayName).Select(x => new GizmoViewModel(ServiceProvider, x, controller)));
