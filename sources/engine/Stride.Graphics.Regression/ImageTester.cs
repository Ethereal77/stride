...
namespace Stride.Graphics.Regression
{
    public static class ImageTester
    {
        ...
        {
            /// <summary>
            ///   Comparison will fail if the image doesn't exist or if it is different.
            /// </summary>
            CompareOnly,

            /// <summary>
            ///   Comparison will fail if the image is different. If no version of it exists yet, it will be created.
            /// </summary>
            CompareOrCreate,
                    ...
                    unsafe
                    {
                        for (int y = 0; y < buffer.Height; ++y)
                        {
                            ...
                            for (int x = 0; x < buffer.Width; ++x, ++pSrc, ++pDst)
                            {
                                ...
                                var r = Math.Abs(src.R - pDst->R);
                                var g = Math.Abs(src.G - pDst->G);
                                var b = Math.Abs(src.B - pDst->B);
                                var a = Math.Abs(src.A - pDst->A);
