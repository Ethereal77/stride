    ...
    public static class HeightmapUtils
    {
        public static bool CheckHeightParameters(Int2 size, HeightfieldTypes heightType, Vector2 heightRange, float heightScale, bool throwExceptionWhenInvalid)
        {
            try
            {
                ...
                {
                    throw new ArgumentException($"{nameof(heightScale)} should be greater than 0.");
                }

                if (heightType == HeightfieldTypes.Float &&
                    !MathUtil.IsOne(heightScale))
                {
        ...
        public static float ConvertToFloatHeight(float minValue, float maxValue, float value) =>
            MathUtil.InverseLerp(minValue, maxValue, MathUtil.Clamp(value, minValue, maxValue)) * 2 - 1;

        public static short ConvertToShortHeight(float minValue, float maxValue, float value) =>
            (short) Math.Round(ConvertToFloatHeight(minValue, maxValue, value) * short.MaxValue, MidpointRounding.AwayFromZero);

        public static byte ConvertToByteHeight(float minValue, float maxValue, float value) =>
            (byte) Math.Round(MathUtil.InverseLerp(minValue, maxValue, MathUtil.Clamp(value, minValue, maxValue)) * byte.MaxValue, MidpointRounding.AwayFromZero);


        public static float[] ConvertToFloatHeights(float[] values, float minValue, float maxValue) =>
            values.Select((v) => ConvertToFloatHeight(minValue, maxValue, v)).ToArray();

        public static float[] ConvertToFloatHeights(short[] values, short minValue = short.MinValue, short maxValue = short.MaxValue) =>
            values.Select((v) => ConvertToFloatHeight(minValue, maxValue, v)).ToArray();

        public static float[] ConvertToFloatHeights(byte[] values, byte minValue = byte.MinValue, byte maxValue = byte.MaxValue) =>
            values.Select((v) => ConvertToFloatHeight(minValue, maxValue, v)).ToArray();


        public static short[] ConvertToShortHeights(float[] values, float minValue, float maxValue) =>
            values.Select((v) => ConvertToShortHeight(minValue, maxValue, v)).ToArray();

        public static short[] ConvertToShortHeights(short[] values, short minValue = short.MinValue, short maxValue = short.MaxValue) =>
            values.Select((v) => ConvertToShortHeight(minValue, maxValue, v)).ToArray();

        public static short[] ConvertToShortHeights(byte[] values, byte minValue = byte.MinValue, byte maxValue = byte.MaxValue) =>
            values.Select((v) => ConvertToShortHeight(minValue, maxValue, v)).ToArray();


        public static byte[] ConvertToByteHeights(float[] values, float minValue, float maxValue) =>
            values.Select((v) => ConvertToByteHeight(minValue, maxValue, v)).ToArray();

        public static byte[] ConvertToByteHeights(short[] values, short minValue = short.MinValue, short maxValue = short.MaxValue) =>
            values.Select((v) => ConvertToByteHeight(minValue, maxValue, v)).ToArray();
