...
public:
    ...
    void ProcessMesh(FbxMesh* pMesh, std::map<FbxMesh*, std::string> meshNames, std::map<FbxSurfaceMaterial*, int> materials)
    {
        ...
        for (int i = 0; i < buildMeshes->Count; ++i)
        {
            ...
            auto vertexBufferBinding = VertexBufferBinding(GraphicsSerializerExtensions::ToSerializableVersion(gcnew BufferData(BufferFlags::VertexBuffer, buffer)), vertexDeclaration, buildMesh->polygonCount * 3, 0, 0);
